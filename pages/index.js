...
const web3_poly = new Web3(Web3.givenProvider);
...
const bet_contract = new web3_poly.eth.Contract(contract_abi1, contractAddress1);
  ...
  useEffect(() => {
    async function load() {
      ...
      //   if (chainId != MAIN_CHAIN_ID) return;
        let lastGameId = await bet_contract.methods.gameId().call();
        let len = Math.min(lastGameId, 8);
        let gameArray = [];
        for (let i = 0; i < len; i++) {
          let game = await bet_contract.methods.games(lastGameId - i - 1).call();
